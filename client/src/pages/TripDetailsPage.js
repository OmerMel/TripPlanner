...
function TripDetailsPage() {
  ...
  const [isFavorite, setIsFavorite] = useState(
    Boolean(
      location.state?.dbId ?? location.state?._id ?? location.state?.isFavorite
    ) || false
  );
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (currentDbId) {
        ...
        setIsFavorite(false);
        // The function that changes the value of trip (setTrip). We send the previous value (prev) and update it.
        // ...prev → Copies all existing attributes of the previous object.
        // dbId: null → changes the value of dbId to null.
        // _id: undefined → deletes or resets _id
        setTrip((prev) =>
          prev ? { ...prev, dbId: null, _id: undefined } : prev
        );

        alert("הטיול הוסר מהמועדפים.");
      } else {
        // Not saved yet => save - POST
        const response = await axios.post("/api/trips/save", trip, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = response.data;

        const savedTrip = data.data;
        setIsFavorite(true);
        ...
        setTrip(
          (prev) =>
            prev
              ? {
                  ...prev, // Copy all existing properties from the previous trip
                  dbId: savedTrip._id, // Set dbId to the new ID returned from the server
                  _id: savedTrip._id, // Update _id to match the saved trip ID
                }
              : prev // If no previous trip, keep state unchanged
        );
